...
export default function EditarVideoPage() {
    ...
    const cargar = async () => {
      try {
        ...
        setForm({
          ...
          duracion:     v.duracion     || '',
          estado:       v.estado       || 'publicado',
        });
        if (v.imagen_url) setPreview(v.imagen_url);
      } catch {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Error al actualizar el video');
    } finally {
      setGuardando(false);
    }
  };
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1 className="page-title">Editar video</h1>
          <p className="page-subtitle">Modifica los campos y guarda los cambios</p>
        </div>
