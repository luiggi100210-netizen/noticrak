    ...
    <>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {videos.slice(0, 4).map((video) => (
          <VideoGridCard key={video.id} video={video} onPlay={() => setSelected(video)} />
        ))}
      </div>
...
function VideoGridCard({ video, onPlay }: { video: Video; onPlay: () => void }) {
  ...
  return (
    <button onClick={onPlay} className="group block text-left w-full">
      <div
      ...
      {video.categoria_nombre && (
        <p className="text-xs font-bold uppercase tracking-wide text-primary-600 mb-1">
          {video.categoria_nombre}
        </p>
      )}
      <h3 className="font-heading text-sm font-semibold line-clamp-2 leading-snug group-hover:text-primary-600 transition-colors mb-1">
        {video.titulo}
      </h3>
